...
async function saveEdit(){
  const d = edit.value
  await api.put(`/defects/${d.id}`, { title: d.title, description: d.description, priority: d.priority, dueDate: d.dueDateLocal ? new Date(d.dueDateLocal).toISOString() : null })
  edit.value = null
  await load()
...
async function saveStatus(){ const s = statusEdit.value; await api.put(`/defects/${s.id}/status`, { status: s.status, statusDescription: s.description }); statusEdit.value=null; await load(); if (infoId.value) await reloadDefect() }
...
async function uploadFiles(){
  ...
  try {
    await api.post(`/defects/${infoId.value}/attachments`, form, { headers: { 'Content-Type': 'multipart/form-data' } })
    filesToUpload.value = []
    await reloadDefect()
  } finally {
...
async function loadProjects(){
  try{
    ...
    if (props.customerOwnedOnly){
      try{
        const { data: prof } = await api.get('/users/get-profile')
