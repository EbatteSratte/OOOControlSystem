...
async function reload(){
  ...
  let profile = null
  try {
    const { data } = await api.get('/users/get-profile')
    profile = data || null
  } catch (e) {
  ...
  // списки
  const [pRes, dRes] = await Promise.all([ api.get('/projects'), api.get('/defects') ])
...
async function downloadExcel(){
  try{
    ...
    const res = await api.get('/reports/customer-excel', { responseType:'blob' })
